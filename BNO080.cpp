// Digital Compass BNO080
// Copyright (C) 2024 https://www.roboticboat.uk
// a4a208d0-9d72-46af-adb4-4becdb1f13d6
//
...
void BNO080::calibrate(byte input)
{
...
void BNO080::calibrateAGM(bool isAccel, bool isGyro, bool isMagnet)
{
  // Allocate memory
...
void BNO080::SetPacketInterval(int reportID, uint16_t milliSeconds)
{
  // Allocate memory
  ...
  if (status != 0){
    ...
    Serial.println("SetPacketInterval status error");
  }
}

float BNO080::Convert2float(int16_t intvalue, float divisor) {
  float fvalue = intvalue;
  return fvalue / divisor;
}

int BNO080::receivePacket()
{
...
int BNO080::processReport(int reportID, uint16_t tmp)
{
...
int BNO080::readRegister(int registerAddress, int* dest)
{
...
int BNO080::readRegisters(int registerAddress, int numBytes, int* dest)
{
  // Initialise the transmit message buffer
  Wire.beginTransmission(_i2cAddress);
...
int BNO080::writeRegister(int registerAddress, int dataByte) {
...
int BNO080::writeBytes(int numBytes, int* dataBytes) {
...
void BNO080::Quanternion2Euler(Quaternion q)
{
...
void BNO080::printStatus(byte level) {
